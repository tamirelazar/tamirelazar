### Tamir Elazar

builds novel interactive systems. Not sure why.

### Here

- [**tslime**](https://github.com/tamirelazar/tslime) · Rust  
  My slimey screensaver app for the terminal.

- **ghostwheel** · TypeScript (private)  
  LLM automation platform for job search, custom CV generation & application management. Will turn public when I find a job (:

### Elsewhere

- [tamir.digital](https://tamir.digital) — full portfolio (visual art · photography · performance)
- [contact@tamir.digital](mailto:tamir.elazar@gmail.com)
